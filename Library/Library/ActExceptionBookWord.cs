        ...
        public void CreateActExceptionBookWord(DataTable dataTable, bool pdf)
        {
            try
            {
                ...
                using (DocX document = DocX.Create(fileName))   //формирование документа в формате docx
                {
                    ...
                    text.Append("\tНастоящий акт составил директор " + AuthorizationForm.FIODirecor + " " + DateTime.Now.ToString("dd.MM.yyyy") + " г. для исключения из фонда библиотеки №127 экземпляров книг на общую сумму " + totalSum + " руб.")
